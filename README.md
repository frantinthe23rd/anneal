...
Arrives at a **front door**: what Anneal is, which models are warm, and a card
per capability. The backdrop was generated by Anneal's own image model and lives
in `assets/`, served by the gateway — the page still makes **no external
requests**. Click the wordmark to return to it.
